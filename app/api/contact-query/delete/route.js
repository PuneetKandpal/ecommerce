...
export async function PUT(request) {
    try {
        ...
        const data = await ContactQueryModel.find({ _id: { $in: ids } }).lean()
        if (!data.length) {
        ...
        if (deleteType === 'SD') {
            await ContactQueryModel.updateMany({ _id: { $in: ids } }, { $set: { deletedAt: new Date().toISOString() } })
        } else {
            await ContactQueryModel.updateMany({ _id: { $in: ids } }, { $set: { deletedAt: null } })
        }
...
export async function DELETE(request) {
    try {
        ...
        await ContactQueryModel.deleteMany({ _id: { $in: ids } })
